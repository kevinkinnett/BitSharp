            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        yield return new KeyValuePair<UInt256, BlockHeader>(blockHash, BlockHeader.FromRawBytes(rawBytes, blockHash));
                    }
                }
            }
        }
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        blockHeader = BlockHeader.FromRawBytes(rawBytes, blockHash);
                        return true;
                    }
                    else
                    {
